// GENERATED WITH generate-modules-sdk
// Warning: Do not edit by hand, all changes will be lost on next execution!

/// <reference types="react" />
import { ISystems } from '../../../../../50-systems/00-SystemsContainer/ISystems';
import { IModuleManifest } from '../../../../../50-systems/ModuleStore/interfaces/IModuleManifest';
interface IModuleStoreModalButtonComponentProps {
    systemsContainer: ISystems;
    shownModule: IModuleManifest;
}
export declare function ModuleStoreModalButtonComponent({
    shownModule,
    systemsContainer,
}: IModuleStoreModalButtonComponentProps): JSX.Element;
export {};
